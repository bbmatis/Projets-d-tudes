...
function genereBoutonDeconnexion(etatCourant) {
  ...
  <div class="navbar-end">
    <div class="navbar-item">
      <div class="buttons">
        <a id="btn-disconnect" class="is-danger button"> Déconnexion </a>
      </div>
    </div>
  </div>`;
    return {
...
function genereListePokemon(etatCourant) {
    //on crée un tableau avec html pour chaque ligne du tableau
    const ligneTableau = etatCourant.pokemons.map((pokemon) => `<tr id="pokemon-${pokemon.PokedexNumber}" class=" ${etatCourant.pokemon && etatCourant.pokemon.PokedexNumber == pokemon.PokedexNumber ? "is-selected" : "" }">
  ...
  <td>${pokemon.Types.join("</br>")}</td>
    ...
    const callbacks = etatCourant.pokemons.map((pokemon) => ({
        [`pokemon-${pokemon.PokedexNumber}`]: {
            onclick: () => {
                console.log("click sur un pokemon", pokemon.PokedexNumber);
    ...
    return {
        html: html,
        //On fusionne les callbacks grâce au reduce  
        callbacks: callbacks.reduce((acc, cur) => ({...acc, ...cur }), {})
    }
}

function genereInfoPokemon(etatCourant) {
    if (!etatCourant.pokemon) return { html: "pokemonInfo", callbacks: {} };
    ...
    return fetch(serverUrl + "/pokemon")
        .then(async(response) => { // async pour pouvoir utiliser await
            if (response.status == 200) {
                const data = await response.json() // await attend la reponse et data contient la liste des pokemons
                return data;
            } else {
